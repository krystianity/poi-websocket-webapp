...
ioMap.prototype.registerStandardPacketTypes = function() {
	...
	this.io.listen('getDefaultLocation', function(packet){
		scope.map.setGEO(JSON.parse(packet), 15);
	});
	...
	//additional listeners that make use of websockets (all broadcasted or cast-ins from the server)
	
	this.io.listen("bc_addPoi", function(packet) {
		console.log("a poi was added by another user " + packet.ID);
		var ie = '<div class="poi_marker" poi_id="' + packet.ID + '"></div>';
		scope.map.addSimplePopup(packet.ID, [packet.lat, packet.lon], ie, scope.onMarkerClick, scope);
	});
	
	this.io.listen("bc_changePoi", function(packet) {
		console.log("a poi was changed by another user " + packet.ID);
	});
	
	this.io.listen("bc_removePoi", function(packet) {
		console.log("a poi was removed by another user " + packet);
		//scope.getMarkerById(packet).remove();
		scope.map.deleteMarkerById(packet); //new way ;)
	});
	
	this.io.listen("bc_clientsTotal", function(packet) {
		console.log("amount of connected app users changed: " + packet);
		if(scope.userDiv !== ""){
			$(scope.userDiv).text(packet);
		}
	});
	
	this.io.listen("bc_poisTotal", function(packet) {
		console.log("amount of pois changed: " + packet);
		if(scope.poiDiv !== ""){
			$(scope.poiDiv).text(packet);
		}
	});
